...
public class Process implements Comparable<Object>{
	...
	public void startCpuBurst() {
		timer.scheduleAtFixedRate(new TimerTask() {
			int counter=0;
			@Override
			public void run() {
				// decrement first element in cpuBurstTime
				cpuBurstTime.set(0, cpuBurstTime.getFirst()-1);
				
				//check if the process finish the first CPU Burst
				if (cpuBurstTime.getFirst() <= 0 ) {
					cpuBurstTime.poll();
					stopCpuBurst();
	...
	public void startIO() {
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			@Override
			public void run() {
				// decrement first element in IOBurstTime
				counter++;
				ioBurstTime.set(0, ioBurstTime.getFirst()-1);
				if (ioBurstTime.getFirst() <= 0 ) {
					ioBurstTime.poll();
					stopIO();
				}
			}
